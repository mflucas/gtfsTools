...
#Now calculate the convex hulls for each break
  ...
  hull_sf <- lapply(hull_sp, FUN=st_as_sf)
  hull_sf <- lapply(hull_sf, FUN=st_set_crs, 4326)
...
#Now refine the display of each isochrone by unioning the alpha hull with the buffered circles around stops:
  ...
  buffer <- st_transform(buffer, 3045)
  hull_sf_buff <- lapply(hull_sf, FUN=st_transform, crs=3045)

  #Create list of unioned isochrones
  buffererd_shapes <- list()
  for(j in 1:(length(breaks))){
    points_buff <- st_buffer(buffer[buffer$cat==j,] , buffer_value)
    points_union <- st_union(points_buff)
    hull_buff <- st_buffer(hull_sf_buff[[j]], buffer_value)

    buffererd_shapes[[j]] <- st_union(points_union, hull_buff)
  ...
  for(i in (length(breaks)):1){
    for(j in (length(breaks)):1){
      if(!(i==j)){
        buffererd_shapes[[i]] <- st_difference(buffererd_shapes[[i]], buffererd_shapes[[j]])
      }
    }
  }

  #Convert back to WGS84
  buffererd_shapes <- lapply(buffererd_shapes, FUN=st_transform, crs=4326)


#Last step: convert it all to one single SP object, which can also be saved as one shapefile
  sp <- lapply(gtfs_isochrones, as, "Spatial")
